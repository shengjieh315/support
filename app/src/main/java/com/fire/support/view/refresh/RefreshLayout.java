...
public class RefreshLayout extends FrameLayout {
    ...
    public RefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (attr == R.styleable.RefreshLayout_enabled_up) {
                    setRefreshEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.RefreshLayout_enabled_down) {
                    setLoadMoreEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.RefreshLayout_style_up) {
                    mHeadStyle = a.getInt(attr, CLASSIC);

                } else if (attr == R.styleable.RefreshLayout_style_down) {
                    mFootStyle = a.getInt(attr, CLASSIC);

                } else if (attr == R.styleable.RefreshLayout_friction) {

                    setFriction(a.getFloat(attr, DEFAULT_FRICTION));

                } else if (attr == R.styleable.RefreshLayout_duration) {

                    mDuration = a.getInt(attr, DEFAULT_DURATION);

                } else if (attr == R.styleable.RefreshLayout_smooth_duration) {

                    mSmoothDuration = a.getInt(attr, DEFAULT_SMOOTH_DURATION);

                } else if (attr == R.styleable.RefreshLayout_smooth_length) {

                    mSmoothLength = a.getInt(attr, DEFAULT_SMOOTH_LENGTH);

                } else if (attr == R.styleable.RefreshLayout_bg_up) {

                    mRefreshBackgroundResource = a.getResourceId(attr, android.R.color.transparent);

                } else if (attr == R.styleable.RefreshLayout_bg_down) {

                    mLoadMoreBackgroundResource = a.getResourceId(attr, android.R.color.transparent);

                } else if (attr == R.styleable.RefreshLayout_is_coo) {
    ...
    protected void onFinishInflate() {
        ...
        if (childCount > 0) {
            ...
            mContentView = findViewById(R.id.refresh_content_view);
        ...
        if (mHeaderView != null && !(mHeaderView instanceof Refresh)) {

            throw new IllegalStateException("mHeaderView  error");
        }
        if (mFooterView != null && !(mFooterView instanceof Refresh)) {
...
//        return !isHeaderRefreshing && mRefreshEnabled && mHeaderView != null && !canChildScrollUp();
    ...
    private Refresh getHeaderInterface() {
        return (Refresh) mHeaderView;
    }

    private Refresh getFooterInterface() {
        return (Refresh) mFooterView;
    }
